...
var PaperStore = class {
	...
	*/
	...
	async proposalFlags(document, proposal) {
		...
		const insertionPositions = /* @__PURE__ */ new Set();
		for (const edit of proposal.edits) {
			...
			switch (command.action) {
				...
				case "decide": {
					...
					if (command.accept) {
						...
						for (const operation of operations.sort((a, b) => b.start - a.start || b.end - a.end)) output = output.slice(0, operation.start) + operation.content + output.slice(operation.end);
