...
function normalizeBlockInsertions(proposal) {
	return {
		...
		edits: proposal.edits.map((edit) => {
			...
			if (blocks.length !== 2 || !first || !second || first.start !== 0 || edit.after.slice(second.end).trim() !== "" || !/^(?:[ \t]*\r?\n){2,}[ \t]*$/.test(edit.after.slice(first.end, second.start))) return edit;
			if (first.text === edit.before && (second.kind === "paragraph" || second.kind === "heading")) return {
				...edit,
				operation: "insert-after",
				after: second.text
			};
			if (second.text === edit.before && (first.kind === "paragraph" || first.kind === "heading")) return {
				...edit,
...
var PaperStore = class {
	...
	constructor(workspaceRoot, maxBytes) {
	...
	*/
	listFiles(path, extension = "md") {
		return this.serial(async () => {
			...
			const entries = (await readdir(directory, { withFileTypes: true })).filter((entry) => !entry.name.startsWith(".") && (entry.isDirectory() || entry.isFile() && entry.name.toLowerCase().endsWith(`.${extension}`))).map((entry) => ({
				name: entry.name,
	...
	*/
	...
	async proposalFlags(document, proposal) {
		...
		for (const edit of proposal.edits) {
			...
			if (block === void 0 || block.text !== edit.before) throw new Error("Edit must match one unchanged base block");
			...
			switch (command.action) {
				...
				case "decide": {
					...
					if (command.accept) {
						...
						const bibliography = await this.bibEntries(document.path);
						const keys = bibliography.files.length ? new Set(bibliography.entries.map((entry) => entry.key)) : null;
						for (const { block, edit } of changes) if (edit.operation || block.kind !== "code" && block.kind !== "html") assertNewCitations(edit.operation ? "" : edit.before, edit.after, keys);
...
async function apply(ctx, config) {
	...
	const storeFor = async (sessionId) => {
		...
		if (!pending) {
			pending = (async () => {
				const store = new PaperStore(root, config.maxBytes);
				await store.start();
				return store;
			})();
			...
			if (toolName === "paper_check") {
				...
				return {
					...
					proposals: document.proposals.filter((p) => p.status === "pending").map((p) => ({
						...
						applicable: !diskChanged && (!p.edits.some((e) => e.operation) || p.baseRevision === document.current.id) && p.edits.every((e) => document.current.blocks.some((b) => b.id === e.blockId && b.text === e.before) && !document.baselines.some((b) => b.blockId === e.blockId && b.locked))
					}))
				};
			}
				...
				items: {
					...
					properties: {
						...
						operation: {
							...
							description: "Use to insert one new paragraph or Markdown heading beside the exact before anchor. Omit for a single-block replacement or an unchanged anchor plus one blank-separated new block."
						}
					}
				}
			}
		...
		async fetch(request) {
			...
			try {
				...
				if (parsed.data.method === "paper-review/list-files") {
					...
					result = {
						ok: true,
						value: await store.listFiles(listing.path)
					};
