...
function checkChanges(proposal, blocks) {
	...
	for (const edit of proposal.edits) {
		for (const [flag, pattern] of patterns) if (JSON.stringify(edit.before.match(pattern) ?? []) !== JSON.stringify(edit.after.match(pattern) ?? [])) flags.add(flag);
	...
	*/
	...
	async proposalFlags(document, proposal) {
		...
		for (const edit of proposal.edits) {
			...
			if (block === void 0 || block.text !== edit.before || edit.before === edit.after) throw new Error("Replacement must match one unchanged base block and contain an actual change");
			...
			switch (command.action) {
				...
				case "decide": {
					...
					if (command.accept) {
						...
						const bibliography = await this.bibEntries(document.path);
						const keys = bibliography.files.length ? new Set(bibliography.entries.map((entry) => entry.key)) : null;
						for (const { block, edit } of replacements) if (block.kind !== "code" && block.kind !== "html") assertNewCitations(edit.before, edit.after, keys);
						output = document.current.text;
						for (const { block, edit } of replacements.sort((a, b) => b.block.start - a.block.start)) output = output.slice(0, block.start) + edit.after + output.slice(block.end);
			...
			if (toolName === "paper_check") {
				...
				return {
					...
					proposals: document.proposals.filter((p) => p.status === "pending").map((p) => ({
						...
						applicable: !diskChanged && p.edits.every((e) => document.current.blocks.some((b) => b.id === e.blockId && b.text === e.before) && !document.baselines.some((b) => b.blockId === e.blockId && b.locked))
					}))
				};
			}
