...
With focus in the review pane, press Cmd+F on macOS or Ctrl+F elsewhere to find text in the current manuscript. The pane switches to **Read** if necessary, highlights visible matches, and scrolls the review pane to the selected match, including later matches in the same paragraph. Enter, Shift+Enter and the arrow buttons move between matches. Escape closes the search and removes its temporary highlights; it does not create saved review highlights or change the manuscript. Search does not take the shortcut from a text input or another pane. Very broad searches stop after 2,000 matches.
