...
焦点在审阅面板内时，在 macOS 按 ⌘F，在其他系统按 Ctrl+F，即可在当前稿件中查找。必要时面板会切到**阅读**页，高亮当前可见的匹配文字，并把审阅面板滚动到当前命中词；同一段里的后续命中词也会准确定位。按回车、Shift+回车或点击箭头可逐项跳转。按 Escape 关闭查找并清除临时高亮；它不会新增持久化高亮或修改正文。焦点在文本输入框或其他面板时，不会接管快捷键。匹配过多时最多显示前 2,000 处。
